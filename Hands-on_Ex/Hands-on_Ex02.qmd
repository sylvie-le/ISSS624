...
abline(v = 0, col = "red")
pacman::p_load(survival, fitdistrplus)
fit <- fitdist(bperm$res, "norm")
plot(fit)
```

The distribution of the simulation result fits a normal distribution. We can conclude that the data does not violate Moran's I assumption of normality.
...
abline(v = 1, col = "red")
fitg <- fitdist(bperm$res, "norm")
plot(fitg)
```

The distribution of the simulation result fits a normal distribution. We can conclude that the data does not violate Geary's C assumption of normality.
...
The fixed distance weight method reveals the hot spots, which are areas with higher GDPPC values among their neighbors, and cold spots, which have lower GDPPC value among their neighbors. However, due to the varying size of the counties, we see some discrepancy between the Gi map (right) and the original map (left). For example, there is a county in Chenzhou that has high GDPPC (above 80,000) but is depicted as a cold area in the Gi map.
